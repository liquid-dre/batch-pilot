 ...
 * `/app` home for growers when Convex is connected:
 *   - Supervisor (foreman): see their farm + wait for the manager's cycle.
 *   - Manager: set up houses + place birds against the contractor's cycle.
 * The contractor's onboarding (add farm, invite managers, schedule cycles) now
 * lives under the Growers nav group (`components/contractor/growerForms.tsx`
 * + the `/app/growers/*` pages), not here.
 */
...
/* ---------------------------------------------------------------- Manager -- */
// The manager runs the farm: sets up houses, places birds against the
// contractor's cycle, and reviews/corrects captured numbers. Foreman invites
// live under Account → Team.
...
/* --------------------------------------------------------------- FarmSetup -- */
// Houses + place-birds for the farm (manager-configured). The place-birds form
// reads the *saved* houses, so houses must be saved before birds can be placed.
// Exported so the standalone "Set up cycle" page (`/app/houses` →
// `components/flock/CycleSetupConvex.tsx`) can reuse the same flow.

export function FarmSetup({ workspace }: { workspace: any }) {
