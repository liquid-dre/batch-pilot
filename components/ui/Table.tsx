...
/**
 * Dense data table for contractor screens (brand-guidelines §6): dark Horizon
 * header row, mono tabular numbers, status pills, compact rows. Composable so
 * callers control columns; numeric cells opt into mono via the `num` prop.
 */
...
export function THead({ children }: { children: React.ReactNode }) {
  return (
    <thead className="bg-brand-900 text-[0.75rem] uppercase tracking-[0.04em] text-brand-100">
      {children}
    </thead>
  );
}
...
export function TR({ className, children, ...props }: React.HTMLAttributes<HTMLTableRowElement>) {
  return (
    <tr
      className={cn(
        "bg-surface transition-colors duration-[var(--dur-fast)] hover:bg-brand-50/60",
        className,
      )}
      {...props}
    >
