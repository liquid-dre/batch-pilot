...
- [x] **ThemeToggle restyled to a labelled azure slider** — adopted the user's segmented-slider reference (`Light · Dark`, a `motion` `layout` pill sliding behind the active side) but **recoloured the pill to solid `bg-brand-700`** (the reference's violet→indigo gradient was dropped — it would have been a second accent + the system's first gradient), icons made conventional (sun=Light, moon=Dark), and the motion gated by `useReducedMotion()` with the house tween. The slider renders in the hero + expanded sidebar; the collapsed rail keeps the compact 36px icon button (`compact` prop). `onDark` uses the white/α glass treatment for the always-dark hero.
...
**Landing hero — eggpit retired for a WebGL line-field (Threads)**
- [x] **Swapped the hero backdrop from the three.js eggpit to React Bits' "Threads"** (`ogl` fragment shader — undulating Perlin lines). `components/marketing/Threads.tsx` is a TS port adapted to conventions: no component-scoped CSS (full-bleed `pointer-events-none` div via `className`), line colour read from a new `--hero-thread` token (`#e6eef8`, a soft azure-white — faint azure over white on the near-black hero) via `getComputedStyle`, and the visibility/`document.hidden` guard + `MAX_RENDER_DIM` DPR cap kept. Rendered in `Landing.tsx` via `dynamic(ssr:false)` (mouse interaction **off**, `amplitude:5`, `distance:0.9`); the blob + logo watermark + scrim stay as static base/legibility.
- [x] **Runs on every device (per the user's call) but honours reduced-motion** — no capability gate, so mobile/low-power render it too; the one guard is `prefers-reduced-motion` → a single static frame (no rAF loop, no listeners), a non-negotiable per AGENTS.md. Ref-write-during-render moved into an effect (the repo's lint forbids it).
- [x] **Dropped `three` for `ogl`** — the eggpit was `three`'s only consumer, so `HeroEggpit.tsx` / `EggpitCanvas.tsx` / `eggpit/engine.ts` and the `--egg-*` tokens were deleted and `three` + `@types/three` removed; `ogl@1.0.11` (bundled TS types) added. `tsc` / lint / `build` / tests (71) green.
- [x] **Live-verified** — the landing `/` is public, so booted `next dev` (with a placeholder Convex URL to clear the pre-existing env gate) and screenshotted the hero with the pre-installed headless Chromium: the azure-white threads render (SwiftShader WebGL), the headline/CTAs stay AA-legible over the scrim, the azure CTA + Light/Dark slider read correctly.

> **Threads decision.** Grilled the scope first: "replace the hero section" meant the animated **backdrop**, not the copy/CTAs (replacing those would gut the page's conversion content). Since `three` was used *only* by the eggpit, the clean move was to retire it wholesale rather than leave two WebGL libs. Colour landed on **faint azure over white** (token-driven) to stay inside the monochrome+azure system without spending the accent. The user chose "run always"; I kept exactly one carve-out — a static frame for `prefers-reduced-motion`, since a continuous full-screen wave is precisely what that setting exists to stop, and the project treats it as non-negotiable.

**Landing hero — WebGL "eggpit" (Ballpit restyled as floating eggs)** _(superseded — retired for Threads, above)_
